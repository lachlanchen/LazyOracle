...
- iOS build 9 and Play internal release 8 carry the layout pass; build 9 is IN_BETA_TESTING on both TestFlight groups, so the public link serves it too.
### 2026-09-22, night — iPad, native foundation
- The iPad (iPad Pro 11-inch, wired to the iMac, which is what the owner calls "7050") is now drivable: Web Inspector and Remote Automation are on, and `safaridriver` reaches it over USB from the iMac. The route is `ssh-lazy imac`, then a WebDriver session with `safari:deviceUDID`.
- Verified on that iPad: focusing the ask bar leaves `visualViewport.scale` at 1 and `scrollWidth` equal to `innerWidth`, so the zoom that widened the interface is genuinely gone on real hardware. Its memory ceiling is 4064 MB in one block, far above what any Tianji model needs.
- Not yet verified there: the model downloads. Two attempts stopped at 3 to 5 per cent with Safari's "Load failed", while the same network from the iMac pulls 20 MB from Hugging Face in 4.8 seconds at 4 MB/s. The failures happened under WebDriver, which is the most likely difference; the honest next step is a download by hand in the installed app. Safari on the iPad is currently stuck paired to an old automation session ("already paired with a different session"), which force-quitting Safari clears.
- Two improvements came out of the investigation anyway: the app now probes both download hosts and uses whichever answers first, because Hugging Face is unusable on some networks and its mirror on others (from this workstation the mirror times out entirely); and it holds a screen wake lock for the length of a download, since a locked phone suspends the transfer and the failure looks like a broken download.
- Native foundation: `src/engine-bridge.ts` exposes all fourteen engine entry points behind one `evaluateJson` call, and `tools/build-engine-bundle.mjs` builds it into `native/shared/lazyoracle-engines.js` (411 KB, no DOM, runs in a bare JS context). `src/engine-bridge.test.ts` checks it returns exactly what the app's own code returns. A native app loads that bundle in JavaScriptCore or QuickJS, so no rule is ever written twice. The order of work is in `docs/plans/native-apps.md`.

### 2026-09-22, night — almanac, agent quality, DeepSeek
- 黄历 (almanac) added as a ninth practice and the app's reason to open daily: for any date it reads the traditional tables through lunar-typescript (宜/忌, 建除十二神, 二十八宿, 黄道黑道十二神, 吉神凶煞, 彭祖百忌, 冲煞, twelve double hours) and answers "can I do this today" for twelve everyday undertakings by quoting the almanac's own word. When neither list names the undertaking it says so and falls back to the day officer and the spirit's road, rather than inventing a verdict. `almanac_day` is also a chat tool.
- Method notes (`src/lib/references.ts`): each practice now carries the order a competent reader works in, with its source named (子平法, 朱熹断例, 八宅, 三停五眼, 通书诸表). They go into every system prompt, which is what stops a model writing pleasant but interchangeable paragraphs.
- Agent quality, following the techniques in AgInTiFlow: temperature 0 everywhere (the same draw now reads the same way), native function calling with `parallel_tool_calls: false` and a text protocol fallback for on-device models, tool results carrying an explicit `ok`, a loop guard against repeated identical calls, an honest-limitation rule near the step cap, and six steps instead of three.
- The chat is unlimited: every message is kept and rendered lazily, while the part that no longer fits the model's context is folded into a summary the model writes itself.
- Two silent failures found and fixed in the relay: it rejected any conversation longer than four messages, and it rejected the assistant's own replies, so every continued chat failed with "no response". It also now retries once with replayed assistant turns demoted, which is DeepSeek's thinking-mode quirk.
- Tianji Cloud runs on DeepSeek: tianji-fast is deepseek-flash (2.4 s) and tianji-pro is deepseek-v4-pro (8.3 s), with the workstation as fallback. The key came from the EchoMind service's environment file on the owner's instruction to use DeepSeek; EchoMind was told and is watching its error rate, and both sessions recommend a separate key.

### 2026-09-22, evening — formal release submitted
- Real devices: the Mix 2S here and the Mi 10 Pro on the 7090 machine both run the app correctly in the browser, with the ask bar focused and the keyboard open, at full width and with no sideways scroll. MIUI refuses adb installs on both (`INSTALL_FAILED_USER_RESTRICTED`), so the signed build could not be side-loaded; the phones run the same code through the web app. The owner's iPad is paired to the iMac but its tunnel is disconnected, so it could not be driven.
- The iPhone SE 3 now reports the memory refusal and switches to the cloud, which the owner confirmed.
- `tools/store-screenshots.py` renders all five store sets from the built app with real content, and `tools/asc-screenshots.py` replaces an App Store set through the API. Both Apple sets now hold eight images each, ending with the agentic chat.
