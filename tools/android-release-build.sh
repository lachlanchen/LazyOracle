...
cd android && ./gradlew --no-daemon --max-workers=6 -q :app:assembleRelease :app:bundleRelease
echo BUILD_OK
ls -la app/build/outputs/bundle/release/*.aab app/build/outputs/apk/release/*.apk
sha256sum app/build/outputs/bundle/release/*.aab app/build/outputs/apk/release/*.apk
