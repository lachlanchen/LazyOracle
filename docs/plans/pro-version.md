...
**Languages: the product and the repository differ on purpose.** The app stays English and Simplified Chinese. The owner settled this on 2026-09-22: "for the model it now only good at zh simplified and English is ok no need to worry it just English and Chinese simplified." A small on-device model is strongest in exactly those two, the engines already carry both, and adding more would weaken the readings it can write. The README, by contrast, is published in eleven languages under `i18n/`, matching the other LazyingArt projects, because that is presentation rather than product.
