...
- Owner's plan: the first store release is the fully local version. Later, Tianji Fast = DeepSeek V4.1 Flash and Tianji Pro = DeepSeek V4.1 Pro through the relay; the DeepSeek API is text-only, so palmistry stays on-device (MediaPipe hand landmarks → structured features → text narration).
- Store reviews (App Store 1.0.0, Play production release 1) are left to run; no new native builds until they finish.
