...
- DeepSeek's chat API is text-only, so it can never do 手相 or 面相 from a photo. Palmistry stays on-device: MediaPipe Hand Landmarker gives 21 points, the engine turns them into structured features, and the language model only narrates the features. Face reading, if it is ever added, follows the same shape with MediaPipe Face Landmarker, or a vision model (Qwen3-VL on the workstation through LazyEdge) behind the relay.
...
- Build 2 carries these changes to the test tracks only: Play internal testing release "2 (1.0.0) local-only default" is published to the tester lists, and iOS 1.0.0 (2) is uploaded to App Store Connect for TestFlight. The reviews already running are untouched: App Store 1.0.0 with build 1 WAITING_FOR_REVIEW, TestFlight external WAITING_FOR_BETA_REVIEW on build 1, Play production release 1 in review in 172 countries.
