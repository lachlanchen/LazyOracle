...
const appName = process.env.VITE_APP_IDENTITY === 'auspice' ? 'Auspice' : 'LazyOracle'
...
export default defineConfig({
  ...
  plugins: [
    react(),
    { name: 'app-identity', transformIndexHtml: (html) => html.replaceAll('LazyOracle', appName) },
    VitePWA({
      registerType: 'autoUpdate',
      includeAssets: ['icons/icon-192.png', 'icons/icon-512.png', 'icons/maskable-512.png'],
      manifest: {
        name: `${appName} — Tarot, BaZi, I Ching`,
        short_name: appName,
